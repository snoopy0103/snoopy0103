...
class Three_dimension():
    def __init__(self, BuyQ, SellQ, StockQ, ewmQ):
        self.ewmQ = ewmQ
        self.tick_1mindata = {}
        self.BuyQ = BuyQ
        self.SellQ = SellQ
    ...
    def start(self):
        while True:
            ...
            #     #Buy & Sell
            #     if self.Code in self.slow_d.keys():
            ...
            #                 self.BuyQ.put([self.Code, self.temptime, self.Price])
            #                 print(f"종목: {self.Code}, 시간: {self.temptime}, 가격: {self.Price}")
