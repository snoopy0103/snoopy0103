...
class RealData_5min():
    ...
    def append(self, 체결시간: str, 현재가: str, 거래량: str, 시가: str, 고가: str, 저가: str, 분5누적거래량: str, 전체누적거래량: str,체결날짜: str) -> None:
        '''
        실시간 데이터를 추가하는 함수
        '''
        self.체결시간+=[체결시간]
    ...
    def last(self):
        ...
        고가 = self.고가[-1]
        저가 = self.저가[-1]
        분5누적거래량 = self.분5누적거래량[-1]
        전체누적거래량 = self.전체누적거래량[-1]
        체결날짜 =self.체결날짜[-1]
        temp_list = [체결시간, 현재가, 거래량, 시가, 고가, 저가, 분5누적거래량, 전체누적거래량, 체결날짜]
        return temp_list
